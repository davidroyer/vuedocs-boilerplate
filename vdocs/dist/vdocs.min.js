...
function createMarkdownFile(config, filename, mdContent) {
    ...
    try {
        _fsExtra2.default.outputFileSync(config.outputDir + '/' + filename + '/README.md', mdContent, "utf8");
    } catch (e) {
        console.log("Cannot write file ", e);
    }
}
...
function createComponentsNavFile(navArray) {
    var dirForFile = './docs/.vuepress';
    var fileName = 'components-nav.json';
    try {
        _fsExtra2.default.outputFileSync(dirForFile + '/' + fileName, JSON.stringify(navArray), "utf8");
    } catch (e) {
        console.log("Cannot write file ", e);
    }
}
...
var init = function init() {
    ...
    (0, _readdirp2.default)(readDirSettings).on('data', function (entry) {
        ...
        if (entry.name.endsWith('.vue')) {
            var navFileName = name.replace('.vue', '');
